  ...
  const setupWebSocket = () => {
    ...
    const fetchDocumentDetails = async () => {
      try {
        ...
        if (
          data.statusCode === "ANALYZING" ||
          data.statusCode === "AI_QUEUED"
        ) {
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    if (!id) return;
                    const url =
                      await documentService.getDocumentDownloadUrl(id);
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      if (!id) return;
                      const url =
                        await documentService.getDocumentReportUrl(id);
                      const link = window.document.createElement("a");
                      link.href = url;
                      link.setAttribute(
                        "download",
      ...
      <div className={styles.content}>
        ...
        <div className={styles.leftPanel}>
          ...
          <div className={styles.card}>
            ...
            <div className={styles.badgeList}>
              {document.derived_documents &&
                Object.entries(document.derived_documents).map(
                  ([type, count], index) => (
              ...
              {(!document.derived_documents ||
                Object.keys(document.derived_documents).length === 0) && (
                <p className={styles.emptyMessage}>
                  No derived documents found.
                </p>
              )}
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.form_id) {
        ...
        setDocumentTypes(
          docTypeRes.map((t: any) => ({ id: t.id, name: t.name })),
        );
                    ...
                    <div className={styles.editInterface}>
                      ...
                      {field.label.toLowerCase() === "client" ? (
                        ...
                        <textarea
                          ...
                          value={val || ""}
                          onChange={(e) =>
                            handleFieldChange(fieldId, e.target.value, label)
                          }
                          rows={3}
                        />
